...
sub new {
  ...
  my $block = bless { DOCUMENT => undef,  # hash generated by XML::Simple
		      ...
		      DTD      => undef,
		      OBS      => 0       # The current <Observation> tag we're inspecting
		    }, $class;
...
sub exposure_time {
  ...
  if (@_) {
     ...
     $exposure =~ s/^\s*//;
     $exposure =~ s/\s*$//;
     $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Schedule}->{Exposure}->{content} = $exposure;
  ...
  my $exposure = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Schedule}->{Exposure}->{content};
  if ( defined $exposure ) {
     $exposure =~ s/^\s*//;
     $exposure =~ s/\s*$//;
...
sub exposure_units {
  my $self = shift;
  return $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Schedule}->{Exposure}->{units};
}
...
sub end_time{
   my $self = shift;
   return $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Schedule}->{TimeConstraint}->{EndDateTime};
}
...
sub ra_format {
  ...
  if (@_) {
    $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Target}->{Coordinates}->{RightAscension}->{format} = shift;
  }
  return $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Target}->{Coordinates}->{RightAscension}->{format};
}
...
sub dec_format {
  ...
  if (@_) {
    $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Target}->{Coordinates}->{Declination}->{format} = shift;
  }
  return $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Target}->{Coordinates}->{Declination}->{format};
}
...
sub equinox {
  ...
  if (@_) {
    $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{Target}->{Coordinates}->{Equinox} = shift;
  }
...
sub data {
  ...
  if (@_) {
     my @array = @_;
     $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData} = [];
     foreach my $i ( 0 ... $#array ) {
        my %hash = %{$array[$i]};

	# Images
	if ( defined $hash{URL} ) {
	   $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$i]->{content} = $hash{URL};
	   $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$i]->{delivery} = "url";
...
sub image_delivery {
  ...
  my @output;
  foreach my $j ( 0 .. $#{$self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}} ) {
    my $delivery = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$j]->{delivery};
    $output[$j] = $delivery;
  }
  return @output;
}
...
sub image_type {
  ...
  my @output;
  foreach my $j ( 0 .. $#{$self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}} ) {
    my $type = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$j]->{type};
    $output[$j] = $type;
  }
  return @output;
}
...
sub image_reduced {
  ...
  my @output;
  foreach my $j ( 0 .. $#{$self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}} ) {
    my $reduced = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$j]->{reduced};
    $output[$j] = $reduced;
  }
  return @output;
}  
...
sub catalogue_type {
  ...
  my @output;
  foreach my $j ( 0 .. $#{$self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}} ) {
    my $type = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$j]->{ObjectList}->{type};
    $output[$j] = $type;
  }
  return @output;
}
...
sub header_type {
  ...
  my @output;
  foreach my $j ( 0 .. $#{$self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}} ) {
    my $type = $self->{DOCUMENT}->{Observation}[$self->{OBS}]->{ImageData}[$j]->{FITSHeader}->{type};
    $output[$j] = $type;
  }
  return @output;
}
...
=item B<number_of_observations>

Find out how many <Observation> blocks are present

   my $num = $object->number_of_observations( );

typically there will only be 1.
