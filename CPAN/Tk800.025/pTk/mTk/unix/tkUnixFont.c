 ...
 *
 * RCS: @(#) $Id: tkUnixFont.c,v 1.1 2003/09/28 14:55:01 aa Exp $
 */
...
static void		DrawChars _ANSI_ARGS_((Display *display,
			    ...
			    int y));
static int		GetControlCharSubst _ANSI_ARGS_((unsigned c, char buf[4]));
...
GetControlCharSubst(c, buf)
    unsigned	c;		/* The control character to be replaced. */
