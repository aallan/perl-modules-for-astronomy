 ...
 *
 * RCS: @(#) $Id: tkButton.c,v 1.1 2003/09/28 14:54:57 aa Exp $
 */
#include "tkButton.h"
#include "default.h"
#include "tkPort.h"
...
Tk_ConfigSpec tkpButtonConfigSpecs[] = {
	...
	|RADIO_BUTTON_MASK},
    {TK_CONFIG_LANGARG, "-height", "height", "Height",
    ...
    char **argv;		/* Argument strings. */
{
	...
	if (butPtr->type == TYPE_CHECK_BUTTON) {
	    if (Tcl_SetVarArg(interp, butPtr->selVarName, butPtr->offValue,
	...
	if (Tcl_SetVarArg(interp, butPtr->selVarName, butPtr->onValue,
		TCL_GLOBAL_ONLY|TCL_LEAVE_ERR_MSG) == NULL) {
	    result = TCL_ERROR;
	}
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
	...
	if (butPtr->selVarName == NULL) {
            butPtr->selVarName = LangFindVar(interp,butPtr->tkwin, Tk_Name(butPtr->tkwin));
	}
	...
	if (value != NULL) {
	    if (LangCmpArg(value, butPtr->onValue) == 0) {
		butPtr->flags |= SELECTED;
	    }
	} else {
	    if (Tcl_SetVarArg(interp, butPtr->selVarName,
		    (butPtr->type == TYPE_CHECK_BUTTON) ? butPtr->offValue : NULL,
		    TCL_GLOBAL_ONLY|TCL_LEAVE_ERR_MSG) == NULL) {
		return TCL_ERROR;
	    }
	}
	...
	value = LangString(Tcl_GetVar(interp, butPtr->textVarName, TCL_GLOBAL_ONLY));
	if (value == NULL) {
	...
	if (Tk_GetPixels(interp, butPtr->tkwin, LangString(butPtr->widthString),
		&butPtr->width) != TCL_OK) {
	    widthError:
	    Tcl_AddErrorInfo(interp, "\n    (processing -width option)");
	    return TCL_ERROR;
	}
	if (Tk_GetPixels(interp, butPtr->tkwin, LangString(butPtr->heightString),
    ...
    register TkButton *butPtr;		/* Information about button. */
{
    if (butPtr->type == TYPE_CHECK_BUTTON) {
	if (butPtr->flags & SELECTED) {
	    if (Tcl_SetVarArg(butPtr->interp, butPtr->selVarName, butPtr->offValue,
		    TCL_GLOBAL_ONLY|TCL_LEAVE_ERR_MSG) == NULL) {
		return TCL_ERROR;
	    }
	} else {
	    if (Tcl_SetVarArg(butPtr->interp, butPtr->selVarName, butPtr->onValue,
		    TCL_GLOBAL_ONLY|TCL_LEAVE_ERR_MSG) == NULL) {
		return TCL_ERROR;
	    }
	}
	...
	return LangDoCallback(butPtr->interp, butPtr->command, 0, 0);
    }
    return TCL_OK;
}

