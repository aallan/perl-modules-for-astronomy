...
#  RCS: @(#) $Id: Photo.pod,v 1.1 2003/09/28 14:55:02 aa Exp $
#  
...
I<format-name> and which has the capability to write an string.
If this option is not given, the data is returned in the default
format as accepted by I<$image>-E<gt>B<put>.

=item B<-from> =E<gt> I<x1 y1 ?x2 y2?>

Specifies a rectangular region of I<$image> to be written to the
string.  If only I<x1> and I<y1> are specified, the region
...
Sets pixels in I< imageName> to the data specified in
I<data>. This command first searches the list of image file
format handlers for a handler that can interpret the data
in I<data>, and then reads the image in I<filename> into
I<imageName> (the destination image). The following options
may be specified:

=over 4

=item B<-format >I<format-name>
...
format handler to read the data. Otherwise I<data> is used to form a two-dimensional array of pixels
that are then copied into the I<$image>. I<data> is structured
...
If this option, the size of I<imageName> will be reduced, if
necessary, so that the region into which the image file data are read
is at the bottom-right corner of the I<imageName>.  This option
...
I<imageName> into which data from I<filename> are to be read.
The default is (0,0).
...
If this option, the size of I<$image> will be reduced, if
necessary, so that the region into which the image file data are read
...
example, if the user specifies B<-format gif>, then a handler
